import { InputHTMLAttributes, ChangeEvent } from 'react';
import { formatReal } from '../../../util/money';
import { FormatUtils } from '@4us-dev/utils';

const formatUtils = new FormatUtils();

interface InputClienteProps extends InputHTMLAttributes<HTMLInputElement> {
    id: string;
    onChange?: (e: ChangeEvent<HTMLInputElement>) => void;
    label: string;
    columnClasses?: string;
    error?: string;
    formatter?: (value: string) => string;
}

const InputCliente: React.FC<InputClienteProps> = ({
    id,
    formatter,
    onChange,
    label,
    columnClasses,
    error,
    ...inputClienteProps
}: InputClienteProps) => {
    //console.log("Bruno inputProps: ", inputProps);
    //console.log("Bruno onChange: ", onChange);

    const onInputChange = (event) => {
        const value = event.target.value;
        const name = event.target.name;

        const formattedValue = (formatter && formatter(value as string)) || value;

        onChange({
            ...event,
            target: {
                name,
                value: formattedValue                
            }
        });
    }

    return (
        <div className={`field column ${columnClasses}`}>
            <label className="label" htmlFor={id}>{label}</label>
            <div className="control">
            <input className="input" 
                    onChange={onInputChange}
                    id={id} {...inputClienteProps} 
            />
                {error &&
                    <p className="help is-danger">{error}</p>
                }
            </div>
        </div>
    )
}

export default InputCliente;

export const InputMoney: React.FC<InputClienteProps> = (props: InputClienteProps) => {
    return (
        <InputCliente {...props} formatter={formatReal} />
    )
}

export const InputCPF: React.FC<InputClienteProps> = (props: InputClienteProps) => {
    return (
        <InputCliente {...props} formatter={formatUtils.formatCPF} />
    )
}

export const InputTelefone: React.FC<InputClienteProps> = (props: InputClienteProps) => {
    return (
        <InputCliente {...props} formatter={formatUtils.formatPhone} />
    )
}

export const InputDate: React.FC<InputClienteProps> = (props: InputClienteProps) => {

    const formatData = (value: string) => {
        if(!value){
            return '';
        }

        const data = formatUtils.formatOnlyIntegers(value);
        const size = value.length;

        if(size <= 2){
            return data;
        }

        if(size <= 4){
            return data.substr(0, 2) + "/" + data.substr(2, 2);
        }

        if(size <= 6){
            return data.substr(0,2) + "/" + data.substr(2,2) + "/" + data.substr(4,2)
        }
    }

    return (
        <InputCliente {...props} maxLength={10} formatter={formatData} />
    )
}